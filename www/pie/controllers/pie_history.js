(function () {
    ...
    function thisFn(
        ...
        $scope.his = [{
            ...
            content: '比如把图灵机器人的apiKey放到APP的自定义字段tulingkey,那么就可以通过ext_httpProxy接口调用，把图灵api的路径写作/openapi/api?key={{tulingkey}}&info=你好,这样项目工场服务器会用你的APP扩展自定义属性替换双大括号{{}}中对应的字段，替你发送请求并把结果返回给你，这样就无须服务器就能调用接口，也不用担心key密钥泄漏了！（ext接口规范请参照【使用帮助】公共接口部分）',
